...
import { sendChangePasswordConfirmationEmail } from "@/app/api/utils/config";
...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        ...
        const results = await client.query(`UPDATE students SET password = $1 WHERE hashed_id = $2 RETURNING *`, [
         hashedPassword,
         hashed_id,
        ]);
